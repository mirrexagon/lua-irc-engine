...
For consistency, there is a `RAW` sender which just returns its argument. I prefer using `irc:send("RAW", ...)` as opposed to `irc:send_raw(...)` because it makes `irc.send` a consistent way to send things.
